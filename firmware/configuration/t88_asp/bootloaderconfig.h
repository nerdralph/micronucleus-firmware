 ...
 *
 * Controller type: ATtiny 88 - 12Mhz on USBasp PCB
 * Configuration:   Default configuration  + LED
 *       USB D- :   PD2
 *       USB D+ :   PD1
 *       Entry  :   POWER_ON
 *       LED    :   ACTIVE_HIGH on D0
 *       OSCCAL :   RESTORE_DEFAULT (8Mhz)
 * Note: can use 12 MHz V-USB without PLL due to stable RC-osc in ATTiny88
...
#define USB_INTR_CFG            PCMSK0 // Pin interrupt enable register
...
#define USB_INTR_ENABLE_BIT     PCIE0 // Bit position in global interrupt enable register
#define USB_INTR_PENDING        PCIFR // Register to read interrupt flag
#define USB_INTR_PENDING_BIT    PCIF0 // Bit position in register to read interrupt flag
...
#define ACTIVE_LOW  2
#define ACTIVE_HIGH_PU 3

#define LED_MODE    ACTIVE_HIGH_PU

#define LED_DDR     DDRC
#define LED_PORT    PORTC
#define LED_PINREG  PINC
#define LED_PIN     PC2
...
#elif LED_MODE==ACTIVE_HIGH_PU
  #define LED_INIT(x)
  #define LED_EXIT(x)
  #define LED_MACRO(x)  if ( x & 0x4c ) {LED_PINREG |= _BV(LED_PIN);}
